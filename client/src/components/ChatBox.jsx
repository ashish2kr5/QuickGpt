...
const ChatBox = () => {
  ...
  const {theme,selectedChat,user,axios,token,setUser} = useAppContext();
    ...
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      <form  onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>
        <select onChange={(e)=>setMode(e.target.value)} value={mode} className='text-sm pl-3 pr-2 outline-none'>
          ...
          <option className='dark:bg-purple-800' value="text">Text</option>
          <option className='dark:bg-purple-800'  value="image">Image</option>
